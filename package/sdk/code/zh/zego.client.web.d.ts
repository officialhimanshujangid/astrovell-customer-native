...
import { ZegoEvent, ZegoCapabilityDetection, ZegoDeviceInfos, ZegoDeviceInfo, ZegoLocalStreamConfig, ZegoMixStreamAdvance, ZegoMixStreamConfig, ZegoPublishStreamAudioConfig, ZegoPublishStreamConfig, ZegoServerResponse, ZegoWebPlayOption, ZegoWebPublishOption, ZegoCheckSingleType, ZegoElectronScreenSource, ZegoEffectsBeautyParam, ZegoSEIConfig, ZegoPublishStats, ZegoPlayStats, SoundLevelDelegateOptions, ZegoLowlightEnhancementMode, ZegoAutoSwitchDeviceConfig, BackgroundBlurOptions, BackgroundVirtualOptions, Segmentation, ZegoGeoFenceType, ZegoStreamUpdateType, AudioFrameOptions, ZegoAutoMixerTask, ZegoAudioFrame } from './ZegoExpressEntity.web';
import { ZegoScenario } from '../../src/common/zego.entity';
import { ZegoStreamOptions } from '../../src/common/zego.entity.web';
...
export declare class ZegoExpressEngine extends ZegoExpressWebRTMEngine {
     ...
     * @param extraInfo 流附加信息，长度不超过1024字符串。
     */
     ...
     *
     * @param localStream 需要切换 AI 降噪的媒体流或者 createZegoStream创建得到的 ZegoLocalStream实例对象
     * @param mode AI 降噪模式，enableAiDenoise默认为AI，AIBalanced、AIAggressive具有更强的抑制效果。
     */
     ...
     * 设置播放视频流类型
     * @param streamID 流 ID
     * @param streamType 拉流大小流类型 0 为小流，1为大流，2为自动
     * @returns
     */
     ...
     * @returns
     */
    setAudioFrameCallback(streamID: string, callback: ((audioData: ZegoAudioFrame) => any) | null, options?: AudioFrameOptions): Promise<ZegoServerResponse>;
    /**
     *
     * 支持版本：3.8.0 及以上
     *
     * 创建流原始音频数据回调，在 createStream 或 createZegoStream 采集成功后使用
     * @param stream  createStream 创建得到的 stream 或者 createZegoStream创建得到的 ZegoLocalStream实例对象
     * @param callback 接收音频数据的回调，设置为 null 则不再获取音频数据
     * @param options
     * @returns
     */
    setCaptureAudioFrameCallback(stream: ZegoLocalStream | MediaStream, callback: ((audioData: ZegoAudioFrame) => any) | null, options?: AudioFrameOptions): Promise<ZegoServerResponse>;
    /**
     * 获取拉流质量。
     *
     * 支持版本：2.15.0 及以上
