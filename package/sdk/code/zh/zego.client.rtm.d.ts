import { ZegoServerResponse, ZegoUser, ZegoRoomConfig, ZegoRTMEvent, ZegoLogConfig, ZegoInitOptions, ZegoProxyInfo, ZegoSwitchRoomConfig, ZegoRoomSendTransparentMessage } from './ZegoExpressEntity.rtm';
...
export declare class ZegoExpressWebRTMEngine {
     ...
     *
     * @param roomID 房间 ID，最大长度为 128 字节的字符串。仅支持数字，英文字符 和 '~', '!', '@', '#', '$', '', '^', '&', '*', '(', ')', '_', '+', '=', '-', ', ';', '’', ',', '.', '<', '>', '/'。
     * @param command 自定义消息内容，长度不超过 1024 字节。
     * @param toUserIDList 目标用户uerId 数组。
     */
    sendCustomCommand(roomID: string, command: string, toUserIDList: string[]): Promise<ZegoServerResponse>;
    /**
     * 设置房间附加消息
     *
     * Note: 支持版本：1.0.0
     ...
     * 发送透传消息
     *
     * Note: 支持版本：3.12.0 及以上。
     *
     * Note: 详情描述：向房间内发送透传消息。
     *
     * Note: 业务场景：一般用于远程控制信令或用户与用户之间的消息发送。
     *
     * Note: 调用时机：登录房间成功之后。
     *
     * Note: 使用限制：关于此接口的使用限制，请参考 https://doc-zh.zego.im/article/7584 或联系 ZEGO 技术支持。
     ...
     * Note: 安全性提醒：请勿在此接口填写用户敏感信息，包括但不限于手机号、身份证号、护照编号、真实姓名等。
     *
     * Note: 相关回调：当发送的消息， mode 指定为 ZegoRoomTransparentMessageModeOnlyClient 或者 ZegoRoomTransparentMessageModeClientAndServer 可通过 [onRecvRoomTransparentMessage] 接收到发送消息的内容。
     *
     * @param roomID 房间 ID，最大长度为 128 字节的字符串。仅支持数字，英文字符 和 '~', '!', '@', '#', '$', '', '^', '&', '*', '(', ')', '_', '+', '=', '-', ', ';', '’', ',', '.', '<', '>', '/'。
     * @param command 自定义消息内容，长度不超过 1024 字节。
     * @param toUserIDList 目标用户uerId 数组。
